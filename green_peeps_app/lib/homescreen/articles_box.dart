...
class _ArticlesBoxState extends State<ArticlesBox> {
  ...
  final Color _boxColor = const Color.fromRGBO(248, 244, 219, 1);
   _getRandomArticle(AsyncSnapshot<QuerySnapshot> snapshot) {
    final List<String> images = ['https://images.pexels.com/photos/1179229/pexels-photo-1179229.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260',
  ...
  final _rand = Random();
    ...
    List<ArticleItem> allArticles = snapshot.data!.docs.map((doc) => ArticleItem(title: doc["title"], link: doc["link"], randImg: images[_rand.nextInt(images.length)],)).toList();
  ...
  Widget _randomArticle(Stream<QuerySnapshot> articles) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        return Container(
          child: _getRandomArticle(snapshot)
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return _buildArticlesBox(context, _boxPadding, _boxElevation, _boxColor, articles);
  }
}
