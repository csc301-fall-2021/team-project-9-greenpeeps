...
class _ArticlesScreenState extends State<ArticlesScreen> {
  ...
  Widget _articles(Stream<QuerySnapshot> articles) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        return ListView(children: _getArticles(snapshot));
      }
  ...
  _getArticles(AsyncSnapshot<QuerySnapshot> snapshot) {
    return snapshot.data!.docs.map((doc) => ArticleItem(title: doc["title"], link: doc["link"], randImg: images[_rand.nextInt(images.length)],)).toList();
  }
